...
# Inspecting Data
In this part of the analysis, I will be doing do introductory inspection of each data set. For this, I will be using nrow to determine the number of cases in each data set, sorting each data set by descending age, and checking the top 10. This will tell me the 10 oldest players on each team.
...
# Data Wrangling
In order to make further judgements of my data, I need to do some data wrangling. In order to do this, I will be joining all five data sets together into one big set. I will do this using the rbind function, as it lets me merge them all in one step. I will be adding a variable called "Team" that will represent each players' team name. I will also be dropping any cases that have an "NA" value, as they will not help our study.
...
agesAvg <-
  allTeams %>%
  group_by(Age) %>%
  summarise(avgHits = mean(H), avgPA = mean(PA), avgHR = mean(HR), avgRBI = mean(RBI), avgSB = mean(SB), avgOBP = mean(OBP), avgOPSplus = mean(OPSplus))

agesAvg
```

# Data Visualization
In this step, I will try to use several different types of graphs to showcase how these variables are affected by each other. For this chart, I have chosen to use the OPS+ statistic to compare to age. I chose this because OPS+ does the best job of evaluating a player's overall hiting.
...
```{r}
agesAvg %>%
  ggplot(aes(x = Age, y = avgOPSplus)) +
  geom_bar(stat="identity") + ylim(0, 150) + theme(axis.text.x = element_text(angle = 30, hjust = 1)) + geom_line(color = "red")
```